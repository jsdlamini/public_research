...
\node[agnostic] (input) {Audio input\\16kHz mono wav, n=892};
\node[agnostic, below=of input] (routing) {MMS-LID-256 routing\\97.2\% accuracy, 3-way};
\node[patha, below left=14mm and 6mm of routing] (whisper) {Whisper-large-v3 QLoRA\\Lingala + Shona};
\node[pathb, below right=14mm and 6mm of routing] (w2v) {wav2vec2-XLSR-53 LoRA\\Luganda};
\node[patha, below=of whisper] (ens1) {SWA + best checkpoint\\Confidence-selected};
\node[pathb, below=of w2v] (ens2) {SWA + best checkpoint\\Confidence-selected};
\node[agnostic, below right=14mm and 0mm of ens1, xshift=18mm] (norm) {Text normalization\\Lowercase-only, jiwer-aligned};
\node[agnostic, below=of norm] (out) {Output CSV\\892 predictions};
...
\draw[arr] (norm) -- (out);
\end{tikzpicture}
\caption{Deployed inference pipeline. Solid boxes indicate the Lingala/Shona path; heavy-bordered boxes indicate the Luganda path; dashed boxes indicate language-agnostic stages.}
\label{fig:pipeline}
\end{figure}
